<div class="max-w-4xl mx-auto p-8">
    <div class="flex flex-col md:flex-row gap-6">
        ...
        <div class="md:w-64 shrink-0">
            <div class="linear-card" style="background: var(--linear-bg-secondary); border: 1px solid var(--linear-border); border-radius: 12px; padding: 1.5rem;">
                <h3 class="text-sm font-semibold mb-4" style="color: var(--linear-text-primary);">Try asking</h3>
                <div class="space-y-2">
                    @php
                        $sampleQuestions = [
                            'What is the vacation policy?',
                            'How does service injection work in Laravel?',
                            'What are the best practices for error handling?',
                            'How do I use Eloquent relationships?',
                        ];
                    @endphp

                    @foreach ($sampleQuestions as $sample)
                        <button
                            wire:key="sample-{{ $loop->index }}"
                            wire:click="askSampleQuestion('{{ $sample }}')"
                            class="w-full text-left px-3 py-2 rounded-md text-sm transition-all"
                            style="color: var(--linear-text-secondary);"
                            onmouseover="this.style.background='var(--linear-bg-hover)'; this.style.color='var(--linear-text-primary)'"
                            onmouseout="this.style.background='transparent'; this.style.color='var(--linear-text-secondary)'"
                        >
                            {{ $sample }}
                        </button>
                    @endforeach
