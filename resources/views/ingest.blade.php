...
<head>
    ...
    <title>Ask My Doc - Ingest Document</title>
...
<body style="background: var(--linear-bg-primary); color: var(--linear-text-primary);">
    <div class="min-h-screen">
        ...
        {{-- Main Content --}}
        <main>
            @livewire('ingest-document')
        </main>
    </div>

    @livewireScripts
    <script>
        // Listen for document ingested event to redirect to Ask page
        window.addEventListener('livewire:initialized', () => {
            Livewire.on('document-ingested', (event) => {
                setTimeout(() => {
